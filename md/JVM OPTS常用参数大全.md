...
- -XX:MaxMetaspaceSize=n 设置元空间大小（JDK1.8）
- -XX:metaspaceSize=n 设置元空间大小（JDK1.8）
...
- -Xloggc: 指定GC log的位置
- -XX:GCLogFileSize=n 设置滚动日志文件的大小，必须大于8k
- -XX:+UseGCLogFileRotation 打开GC日志滚动记录功能
- -XX:NumberOfGCLogFiles=n 设置滚动日志文件个数
...
查看JVM参数默认值可以通过java -XX:+PrintFlagsFinal -version，也可以使用 grep JVMParamName获取具体某个参数值。
